...
PATH_TO_LABELS = os.path.join('object_detection/data', 'label.pbtxt') ### CWH: Add object_detection path
...
def get_objects(image, threshold=0.5):
    ...
    # print("detected %s Food in image above a %s score" % (obj_above_thresh, threshold))
    # print("Image Size = {}".format(image_np_expanded.shape))
    output = []
    check_duplicate = {}
    for c in range(0, len(classes)):
        ...
        if scores[c] >= threshold:      # only return confidences equal or greater than the threshold
            ...
            if(class_name in  check_duplicate):
                continue
            check_duplicate[class_name] = True
            item = Object()
            item.name = 'Object'
            ...
            output.append(item)
    # a = [ob.__dict__ for ob in output]
